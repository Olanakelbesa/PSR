...
export const proposalSchema = z
  .object({
    ...
    budgetRequested: z.number({ required_error: "Budget amount is required" }),
    proposalType: z.string().min(1, "Proposal type is required"),
    subProposalTypeId: z.string().optional(),
    grantCallId: z.string().min(1, "Grant call is required"),
    submissionLevel: z.string().min(1, "Submission level is required"),
    officeToSubmit: z.string().min(1, "Office to submit is required"),
    receivingOffice: z.string().min(1, "Receiving office is required"),
    thematicArea: z.string().optional(),
    thematicAreas: z.array(z.string()).min(1, "At least one thematic area is required"),
