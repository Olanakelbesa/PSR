...
const upcomingDeadlines = [
  { title: 'TVET Checklist Review', days: 2, status: 'Urgent', color: 'text-red-500' },
  { title: 'PSR Ratification Board', days: 5, status: 'Scheduled', color: 'text-blue-500' },
  { title: 'ROC Proposal Deadline', days: 12, status: 'Incoming', color: 'text-emerald-500' },
]
      ...
      <CardContent className="p-5">
        ...
        <div className="mt-4">
          ...
          <div className="flex items-center gap-1 mt-2">
            ...
            <span className="text-[10px] text-muted-foreground font-medium uppercase">{stat.changeLabel}</span>
          </div>
        </div>
      </CardContent>
    </Card>
  ...
  return (
    <Card className="shadow-sm border-primary/10">
      <CardHeader className="pb-2 border-b bg-muted/30">
        ...
        <CardDescription>Document throughput across all approval stages</CardDescription>
      </CardHeader>
        ...
        <div className="space-y-3.5">
          {approvalPipelineData.map((item, index) => (
            <div key={item.stage} className="flex items-center gap-3">
              ...
              <span className="text-xs font-medium text-muted-foreground w-32 truncate">{item.stage}</span>
        ...
        <div className="flex flex-col items-center">
          <div className="relative w-40 h-40">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] font-bold text-muted-foreground uppercase">Total Active</span>
            </div>
          </div>
          <div className="w-full mt-6 space-y-2">
            {workflowOverviewData.map((item) => (
              <div key={item.name} className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground/70 text-[10px]">({item.percentage}%)</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <CardContent className="pt-6">
        <div className="space-y-5">
          {upcomingDeadlines.map((deadline) => (
            <div key={deadline.title} className="flex items-start justify-between group">
              <div className="flex items-start gap-3">
                <div className="p-2 bg-muted rounded-lg group-hover:bg-primary/5 transition-colors">
                   <Calendar className="h-4 w-4 text-muted-foreground group-hover:text-primary transition-colors" />
                </div>
                <div>
                  <p className="text-sm font-bold leading-none">{deadline.title}</p>
                  <p className={cn("text-[10px] font-bold uppercase mt-1.5", deadline.color)}>{deadline.status}</p>
                </div>
              </div>
        ...
        <Button variant="ghost" className="w-full mt-6 h-9 text-xs font-bold uppercase tracking-wider hover:bg-primary/5 text-primary">
        ...
        <div className="space-y-4 relative">
          ...
          {recentActivities.map((activity, index) => (
            <div key={index} className="flex items-start gap-4 relative z-10">
              <div className={cn("w-2 h-2 rounded-full mt-1.5 shrink-0 shadow-sm", iconColors[activity.icon])} />
              <div className="flex-1 min-w-0">
                <p className="text-xs leading-relaxed">
                  {activity.user && <span className="font-black text-foreground">{activity.user} </span>}
...
function ThematicAreaChart() {
  ...
  return (
    <Card className="shadow-sm border-primary/10">
      <CardHeader className="pb-2 border-b bg-muted/30">
        <CardTitle className="text-sm font-bold uppercase tracking-wider">Policies by Thematic Area</CardTitle>
      </CardHeader>
      <CardContent className="pt-6 pb-2">
        <div className="flex flex-col items-center">
          <div className="relative w-36 h-36">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-xs font-bold text-muted-foreground uppercase">Registry</span>
            ...
            {thematicAreaData.map((item) => (
              <div key={item.name} className="flex items-center gap-2 text-[10px]">
                <div 
                  ...
                  style={{ backgroundColor: item.color }} 
                />
                <span className="text-muted-foreground font-bold truncate flex-1 uppercase tracking-tight">{item.name}</span>
    ...
    <Card className="shadow-sm border-primary/10 h-full">
      ...
      <CardContent className="pt-6">
        <div className="h-[280px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={researchTrendData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
