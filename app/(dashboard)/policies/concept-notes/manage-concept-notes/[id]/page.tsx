            ...
            <TabsContent value="document" className="mt-6">
              <Card className="shadow-lg border-primary/20 overflow-hidden">
                <PdfViewer
                  ...
                  title="Policy Framework Concept Note"
                  className="h-[900px]"
                />
              </Card>
            </TabsContent>
            ...
            <TabsContent value="feedback" className="mt-6 space-y-8">
              <div className="space-y-12">
                {note.reviews && note.reviews.length > 0 ? (
                  Object.entries(
                    note.reviews.reduce((acc: any, review: any) => {
                    ...
                    .map(([version, reviews]: [string, any], vIndex) => (
                      <div key={version} className="relative">
                        {/* Version Separator */}
                        <div className="flex items-center gap-4 mb-6">
                          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary text-primary-foreground shadow-sm">
                            <GitBranch className="h-3.5 w-3.5" />
                            <span className="text-xs font-black uppercase tracking-wider">{version}</span>
                          </div>
                          <div className="h-px flex-1 bg-gradient-to-r from-primary/20 to-transparent" />
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                            {reviews.length} Expert Assessment{reviews.length !== 1 ? "s" : ""}
                          </span>
                        </div>

                        {/* Reviews for this version */}
                        <div className="grid gap-6 pl-2 border-l-2 border-primary/5 ml-4">
                          {reviews.map((review: any) => (
                            <Card
                              key={review.id}
                              className="shadow-sm border-primary/10 bg-background overflow-hidden hover:shadow-md transition-shadow duration-300"
                            >
