...
> = {
  pending_submission: {
    ...
    icon: Clock,
    description: "You need to complete and submit this clearance application to the IRB committee.",
  },
  pending_review: {
    label: "Pending Review",
    className: "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 border-blue-200 dark:border-blue-800",
    icon: ShieldCheck,
    description: "Your application is currently under formal review by the IRB Ethics Committee.",
  },
  approved: {
    label: "Approved",
    ...
    icon: CheckCircle2,
    description: "Your IRB ethical clearance has been approved and issued.",
  },
  rejected: {
    label: "Rejected",
    className: "bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300 border-rose-200 dark:border-rose-800",
    icon: XCircle,
    description: "Your application was not approved. Please review committee feedback and resubmit.",
  },
  resubmitted: {
    label: "Resubmitted",
    ...
    icon: RefreshCcw,
    description: "You have resubmitted this application. Awaiting committee re-evaluation.",
  },
};
...
export default function SubmissionDetailPage() {
  ...
  const proposalIdStr = clearance?.proposal ? String(clearance.proposal) : clearance?.proposalId ? String(clearance.proposalId) : "";
  ...
  const documentList = useMemo(() => {
    ...
    // 1. Proposal File from Proposal detail or clearance props
    const propFile = (clearance as any).proposalFile || (clearance as any).proposal_file || proposalDetail?.proposalFile || (proposalDetail as any)?.proposal_file;
    if (propFile) {
      list.push({
        key: "proposal-file",
        label: "Proposal File",
  ...
  if (isLoading) {
    return (
      <PageContainer title="Loading IRB Submission...">
  ...
  if (!clearance) {
    return (
      <PageContainer
        ...
        description="The requested IRB submission could not be loaded."
        actions={
          <Button variant="outline" onClick={() => router.push("/research/irb-clearance/my-submissions")}>
        ...
        <Card className="border-l-4 border-l-amber-500 bg-amber-50 dark:bg-amber-950/20">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-amber-800 dark:text-amber-300">The IRB submission details could not be loaded. Please try again or contact support.</p>
            </div>
          </CardContent>
        </Card>
      </PageContainer>
  ...
  const showSubmitButton = status === "pending_submission" || status === "rejected";
  const hasReviews = Boolean(clearance.reviews && clearance.reviews.length > 0);

  return (
    <PageContainer
      title={clearance.proposalTitle || "IRB Ethical Clearance Submission"}
      description={
        ...
        <div className="flex items-center gap-2">
          ...
          {showSubmitButton && (
            <Button size="sm" className="shadow-xs" asChild>
              <Link href={`/research/irb-clearance/my-submissions/submit/${clearanceId}`}>
                {status === "rejected" ? (
                  <>
    ...
    >
      {/* ── Main Layout: Standard Grid 2-Column ────────────────────────────── */}
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        {/* Main Content Column */}
        <div className="space-y-6 min-w-0">
          <Tabs defaultValue="overview" className="w-full">
            ...
            <TabsContent value="overview" className="pt-5 space-y-6">
              {/* Funding Decision Committee Remarks & Guidance Banner */}
                ...
                <Card className="border-dashed py-16 text-center">
                  <CardContent className="flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-xs text-muted-foreground max-w-sm">
                      No clearance certificates or supporting protocol documents were uploaded with this submission.
                    </p>
                  </CardContent>
                </Card>
              )}
                ...
                <CardContent className="pt-5 space-y-4">
                  {hasReviews ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-center gap-2">
                      ...
                      <p className="font-bold text-muted-foreground text-sm">No Review Comments Yet</p>
                      <p className="text-xs text-muted-foreground max-w-sm">
                        Review history and committee decisions will appear here once formal review begins.
                      </p>
                    </div>
                  )}
                </CardContent>
