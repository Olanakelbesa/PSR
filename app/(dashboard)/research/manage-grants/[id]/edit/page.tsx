...
export default function EditCallPage() {
  ...
  const { data: call, isLoading: isCallLoading } = useGrantCall(id);
  ...
  // Populate data when call detail is loaded
  useEffect(() => {
    if (call && !hasInitialized) {
      setTitle(call.title ?? "");
      ...
      setCurrentYear(call.currentYear ?? "2026");
      
      if (call.proposalTypes) {
        setSelectedTypes(call.proposalTypes.map((t) => String(t.id)));
      }
      if (call.installmentPlans) {
        setInstallments(
          call.installmentPlans.map((ip) => ({
            id: Number(ip.id),
            percentage: Math.round(Number(ip.percentage)),
          })),
        );
      }
  ...
  const removeInstallment = (installmentId: number) => {
    setInstallments(installments.filter((it) => it.id !== installmentId));
  };
  ...
  function buildGrantCallPayload() {
    ...
    if (thumbnail instanceof File) {
      payload.thumbnail_image = thumbnail;
  ...
  async function persistGrantCall(
    ...
    const savePromise = (async () => {
      setErrors(null);
      const saved = await updateGrantCall(id, payload);
      lastSavedSignatureRef.current = signature;

      if (mode !== "autosave" && mode !== "publish") {
        toast.success("Grant call updated");
      }
  ...
  useEffect(() => {
    if (!canAutosave || !hasInitialized) return;
  ...
  if (isCallLoading) {
    return (
      <PageContainer
        ...
        description="Update an existing funding opportunity"
      >
  ...
  return (
    <PageContainer
      title="Edit Grant Call"
      description="Update and configure an existing funding opportunity"
      actions={
        <Button
          variant="outline"
          size="sm"
